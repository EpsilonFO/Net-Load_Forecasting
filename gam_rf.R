...
missing_predictors <- setdiff(all.vars(equation), names(Data0))
if (length(missing_predictors) > 0) {
  stop(paste("Missing predictors in data:", paste(missing_predictors, collapse = ", ")))
}

# Train the GAM model with shrinkage
gam_model <- gam(equation, data = Data0[sel_a,], select = TRUE, gamma = 1.5)

# Make predictions on the test data
gam_forecast <- predict(gam_model, newdata = Data1)

# Calculate RMSE for the training set
rmse_train <- sqrt(mean((Data0$Net_demand[sel_a] - predict(gam_model, newdata = Data0[sel_a,]))^2))
...
write.table(submit, file = "Data/gam_rf.csv", quote = FALSE, sep = ",", dec = '.', row.names = FALSE)
